...
<p>
There are ${totalUploads} uploads.
</p>
...
jQuery(document).ready(function($) {
    ...
    $('.pagination').pagination({
        ...
        onPageClick: function (page, evt) {
            window.document.location = "<c:url value="/admin/uploads" />" + "?page=" + page + "&sortDir=${sortDir}";
        }
    });
    $('.pagination').show();
});
...
<c:forEach begin="1" end="${count}" var="val">
    <li <c:if test="${val == page}">class="active"</c:if>><a href="<c:url value="/admin/uploads?page=${val}&sortDir=${sortDir}" />"><c:out value="${val}"/></a></li>
</c:forEach>
...
<table class="table table-striped">
    <thead>
    <tr>
        <th width="20%">Uploaded on &nbsp;&nbsp;<a href="<c:url value="/admin/uploads?page=${page}&sortDir=${oppSortDir}" />"><i class="fa fa-sort" aria-hidden="true"></i></a></th>
